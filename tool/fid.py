...
def calculate_fid_given_paths(paths, batch_size=50, device = None, dims=2048, num_workers=1,dataset = None):
    if device is None:
...
def calculate_statistics_given_paths(path,batch_size=100, device = None, dims=2048, num_workers=1,transforms=None):
    if device is None:
        device = torch.device('cuda' if (torch.cuda.is_available()) else 'cpu')
    else:
        device = torch.device(device)
